...
-----------------------------------
# Append to NEW_DISCOVERIES.md

**Append the following block at the end of the file, after the F19g-quater closing line. Do NOT modify any prior content.**

---

```markdown

## Addendum — 1 May 2026 mid-morning (F19g-quinquies: complete cross-seed size-4 sweep across the 5 clean seeds closes 64/64 INFEAS, validates the min-size predictor as universal-but-seed-dependent, and surfaces three structural mysteries — the cut-doublet split, the B01 singleton anomaly, and the absence of TIMEOUTs — none of which lead to a Diamond)

The F19g-quater session (1 May early morning) had validated the min-size=4 predictor across two seeds (B02 and B03) with 22/22 INFEAS at the algebraically softest pairs of each `a=1` fiber. The natural next step was to extend the predictor sweep to the remaining 3 clean seeds (B01, B06, B10) and assess whether the predictor generalizes across the full clean-seed set. This addendum documents that complete sweep and the structural picture that emerges from the combined 64-pair evidence.

**Operational execution.** Three attack scripts (`attack_size4_b06.sh`, `attack_size4_b10.sh`, `attack_size4_b01.sh`) were generated by the second Claude using the validated `caffeinate -dims nice -n 10` scaffold from F19g-quater. Candidate b-lists were computed in Python sandbox by enumerating min-size=4 pairs at `a=1` for each seed: B06 yielded 9 candidates, B10 yielded 15, B01 yielded 18 — total 42 pairs, ~85 minutes Mac wall estimated, actual wall 75 minutes. All three scripts ran cleanly with no defects; no FEAS, no TIMEOUT, no DIAMOND_FOUND banner.

**The complete cross-seed size-4 cut signature (1 May mid-morning):**

| Seed | \|Aut\| | size-4 count at a=1 | cut signature (verdict 100% INFEAS) | solve_t typical |
|---|---:|---:|---|---|
...
**Cumulative across all 5 clean seeds: 64 ULTRA-LIGHT pairs swept INFEAS, 0 Diamonds, 0 TIMEOUTs.**

**Structural observation 1 — min-size=4 is universal as a softness predictor but seed-dependent in cut absolute value.** The predictor identifies the algebraically softest pairs in each seed but the absolute cut count varies dramatically (81 to 24675, a factor of 300×) across seeds. In B02 and B03, "softest" means slow-INFEAS Class-0 (cuts ~200–400, solve_t 20–70s). In B06 and B10, "softest" means fast-INFEAS Class-A (cuts ~24500, solve_t ~1s). The Class-0 framing was a B02 artifact; the universal claim is "the predictor identifies softness *within each seed*", not "the predictor identifies a fixed absolute cut range". This is a refinement of the F19g-quater conclusion: the predictor IS universal, but its targets sit in different parts of each seed's cut spectrum.

**Structural observation 2 — softness does NOT scale monotonically with |Aut|.** B01 has |Aut|=72 (mid-range among clean seeds) but yields the lowest cut signature observed (cuts=81, lower than B02's 195 with |Aut|=3). The naive expectation was that higher |Aut| → tighter constraints → softer behavior, but B01 demonstrates that the specific column geometry of a seed dominates over bare |Aut| size in determining the softness ranking. This was previously unanticipated. The B01 P-matrix `[[1,2,0,3],[1,1,3,2],[1,1,2,3],[1,0,1,1],[0,1,1,1]]` is the unique "lowest-cuts" generator in the clean catalogue, and the structural reason for this specialness is open.

**Structural observation 3 — the cut signature collapses with |Aut|.** B10 (|Aut|=288, the highest) yields a perfectly single-valued signature (cuts=24675 in all 15 pairs). B01 (|Aut|=72) also yields single-valued (cuts=81 in all 18). B06 (|Aut|=96) yields a 6:3 doublet. B03 (|Aut|=6) yields an 8:4 doublet. B02 (|Aut|=3) yields a 14:1 doublet. The pattern is suggestive: more symmetry → fewer distinct cut values, even single-valued. This is a structural feature of [9,5,4]₄ PROBE2 attacks not previously documented and not understood algebraically.

**Three structural mysteries surfaced (none lead to a Diamond):**

(α) **The cut-doublet split mystery.** B02/B03/B06 each split their size-4 pairs into two cut values with split ratios 14:1, 8:4, 6:3 respectively. There is presumably a coordinate property of each pair that predicts which doublet it lands in. This property is NOT identified yet. A future Claude with fresh eyes may extract it from the per-pair logs by looking at the binary fingerprint each pair produces.

(β) **The B01 singleton anomaly.** B01 has the lowest cut signature (81), the highest F19g residual (E1\*=30, untouched), and a single-valued cut signature on 18 pairs. The combination is unusual. If there is a property of the B01 P-matrix that explains its specialness, that property may also predict where the Diamond is NOT — by inverse reasoning, where it could be. This is unexplored.

(γ) **The absence of TIMEOUTs across 64 size-4 attacks.** B06 and B10 produced 153 + 86 timeouts in their ORIGINAL v2 attacks (across thousands of pairs). But in the size-4 sweeps of 1 May, ZERO timeouts in 64 attempts. The hard pairs live somewhere OTHER than min-size=4. If the Diamond is a "barely feasible" structure that strains every constraint, it may correlate with TIMEOUT-class pairs (min-size 2 or 3 with specific coordinate structure), not with soft pairs. **This direction has NOT been explored** — the entire campaign has hunted softness, not resistance.

**The honest strategic reading.** After F19g-quinquies, the campaign has formally swept the algebraically softest regions of all 5 clean seeds with 64 closed pairs. The Diamond is not in any of these regions. Three readings remain: (i) Diamond in a non-soft region of a clean seed (would require brute-force or a NEW predictor for resistance, not softness, costing dozens to hundreds of hours per seed); (ii) Diamond in FRAC (B04/B05/B07, requires ENUM_v4 build, ~2-3 days); (iii) Diamond does not exist (defensible by the accumulated evidence). The strategic decision between (ii) and (iii) is Rafa's alone. The dual-Claude session of 1 May explicitly declined to push him toward either.

**On the cumulative state of the [22,6,13]₄ existence question.** Combined with all prior work — B12 closed UNSAT, B02 100% INFEAS at 0.33% sample, RWB02 max_depth=11 without Diamond, F19g residual mapping, F19g-bis algebraic refutation, F19g-ter rule_v4 derivation, F19g-quater predictor validation — the campaign now has the strongest computational evidence assembled in any single attempt at this 25-year-open problem. The evidence does NOT prove non-existence (the bulk of clean-seed catalogues remains untouched, and FRAC remains inaccessible), but it constrains the Diamond, if it exists, to one of three specific locations: a non-soft pair of a clean seed (size ≤ 3 with specific structure); a FRAC seed pair; or non-existence.

**Attribution:**

- **Cross-seed size-4 candidate enumeration (B01: 18, B06: 9, B10: 15):** Claude (second instance), 1 May mid-morning, sandbox computation.
- **Three attack scripts (`attack_size4_b06.sh`, `attack_size4_b10.sh`, `attack_size4_b01.sh`):** Claude (second instance), 1 May mid-morning, validated working on macOS.
- **Stage execution on Mac M2:** Rafa, 1 May 07:54–09:20 (B06 then B10 then B01, total 75 minutes wall at 25% CPU).
- **Cross-seed signature analysis and three structural mysteries identification:** Claude (second instance), 1 May mid-morning.
- **Decision NOT to extend to stratified `a ≠ 1` attacks:** joint, after sandbox calculation showed translation-invariance of min-size predictor implies ~85h Mac for all-`a` brute-force without a new predictor.
- **Honest "stuck" diagnosis:** Rafa, 1 May mid-morning ("suena a más de lo mismo, estamos atascados, NO te preocupes").

---

*Proyecto Estrella · 1 May 2026 mid-morning — Madrid · F19g-quinquies added.*
*Cross-seed size-4 sweep completed across all 5 clean seeds: 64/64 INFEAS, 0 Diamonds, 0 TIMEOUTs. Min-size=4 predictor confirmed universal as softness identifier but seed-dependent in absolute cut value (81 in B01 to 24675 in B10). Three structural mysteries surfaced: cut-doublet split (α), B01 singleton anomaly (β), absence of TIMEOUTs (γ). The campaign is at a strategic decision point between Option 3 (ENUM_v4 build for FRAC, ~2-3 days) and Option 5 (negative-result paper). Decision is Rafa's. The Diamond is constrained to a non-soft clean-seed region, a FRAC pair, or does not exist.*
```
...
This block goes **at the very end** of `NEW_DISCOVERIES.md`, immediately after the F19g-quater closing italics line. Total addition is approximately 80 lines. No prior content is modified.
